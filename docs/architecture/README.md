...
CPA Orbit is a local-first modular monolith. It deliberately uses one Go control plane for the browser and desktop clients so operational state cannot drift between two backends. The desktop host embeds the production Vue bundle, exposes the same API handler through Wails, and also publishes the Monitor API on loopback for the browser console. Archived subscription JSON remains the durable source of truth; the CLIProxyAPI auth directory is a rebuildable projection.
...
flowchart TB
  ...
  subgraph CONTROL["02 · LOCAL CONTROL PLANE"]
    ...
    SUBS["Subscription Engine<br/><small>archive · dedupe · quota · sync</small>"]
  ...
  subgraph PROJECTION["04 · REBUILDABLE PROJECTION"]
    direction LR
    AUTH[("CPA Auth Pool")]
    CLI["CLIProxyAPI"]
  end
  ...
  EDGE --> MONITOR & SUBS & LUBAN
  MONITOR <--> HISTORY
  MONITOR <--> SETTINGS
  SUBS <--> SETTINGS
  SUBS <--> ARCHIVE
  SUBS -->|"deterministic reconciliation"| AUTH
  AUTH --> CLI
  ...
  class EDGE,MONITOR,SUBS,LUBAN control;
  class SETTINGS,HISTORY,ARCHIVE persistence;
  class AUTH,CLI projection;
```
...
sequenceDiagram
  ...
  alt New subscription
    ...
    opt Sync enabled
      Guard->>Pool: Write safe provider projection
      Pool-->>CPA: Hot-load auth entry
    end
...
flowchart LR
  ...
  BLOCKED["Never returned:<br/>OAuth tokens · CPA key · Luban key"]
  SECRET -.-> BLOCKED
