...
| 订阅资产 | 归档、去重、投影与检查 | `k12/` 是事实源，`cpa/auths/` 是投影 |
| 号池运维 | 网关目标、托管绑定、迁移与 Token 可视化 | `data/control-plane.db`；原始请求明细归 Sub2API |
...
`k12/MMDD` 下的归档是唯一事实源，`cpa/auths` 是提供给 CLIProxyAPI 的可重建投影。健康检查会区分 HTTP 401、HTTP 402、额度耗尽、限流、禁用和未加入活动池等状态。
...
Sub2API 是推荐主订阅池，CPA 是轻量兜底。Orbit 通过官方管理员接口导入 Codex session，持久化目标和部署绑定，并保留有界的 15 分钟用量聚合。启用双网关前请阅读 [Sub2API 号池指南](/zh/guide/sub2api-pool)。
