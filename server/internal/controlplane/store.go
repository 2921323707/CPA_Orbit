...
func (s *Store) UpsertGatewayTarget(ctx context.Context, target GatewayTarget) (GatewayTarget, error) {
	...
	if target.ID == 0 {
		result, err := s.db.ExecContext(ctx, `INSERT INTO gateway_targets
			(kind, name, base_url, admin_key, enabled, is_primary, allow_remote, created_at, updated_at)
			VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
			...
			boolInt(target.Enabled), boolInt(target.Primary), boolInt(target.AllowRemote), formatTime(now), formatTime(now))
		if err != nil {
		...
		result, err := s.db.ExecContext(ctx, `UPDATE gateway_targets SET
			...
			enabled=?, is_primary=?, allow_remote=?, updated_at=? WHERE id=?`,
			...
			boolInt(target.Enabled), boolInt(target.Primary), boolInt(target.AllowRemote), formatTime(now), target.ID)
		if err != nil {
...
func (s *Store) ListGatewayTargets(ctx context.Context) ([]GatewayTarget, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, kind, name, base_url, admin_key<>'', enabled, is_primary, allow_remote, created_at, updated_at FROM gateway_targets ORDER BY is_primary DESC, name`)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&target.ID, &target.Kind, &target.Name, &target.BaseURL, &target.AdminKeyConfigured, &enabled, &primary, &allowRemote, &createdAt, &updatedAt); err != nil {
			return nil, err
		}
...
func (s *Store) gatewayTarget(ctx context.Context, id int64, includeSecret bool) (GatewayTarget, error) {
	...
	var enabled, primary, allowRemote int
	var createdAt, updatedAt string
	err := s.db.QueryRowContext(ctx, `SELECT id, kind, name, base_url, admin_key, enabled, is_primary, allow_remote, created_at, updated_at FROM gateway_targets WHERE id=?`, id).
		Scan(&target.ID, &target.Kind, &target.Name, &target.BaseURL, &secret, &enabled, &primary, &allowRemote, &createdAt, &updatedAt)
	if err != nil {
